...
export default function TechStack({ activeTechFilter, onSelectTech }) {
  ...
  return (
    <section id="tech-stack" className="py-24 relative overflow-hidden bg-[#07070e]/80 border-t border-b border-white/5">
      {/* Background Lighting */}
          ...
          {techStack.map((item, idx) => {
            ...
            return (
              ...
              >
                {/* Highlight Badge */}
                {isSelected && (
                  <span className="absolute top-3 right-3 text-[10px] uppercase font-mono px-2 py-0.5 rounded-full bg-cyan-500 text-black font-bold">
                    Active
                  </span>
                )}

                <div className="flex items-center gap-4 mb-4">
                  <div className={`p-3 rounded-xl ${isSelected ? 'bg-cyan-500 text-black' : 'bg-purple-950/60 text-cyan-400 border border-cyan-500/20'}`}>
                    <IconComponent className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-white tracking-wide">{item.name}</h3>
                    <span className="text-xs text-purple-400 font-mono">{item.category}</span>
                  </div>
                </div>
